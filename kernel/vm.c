  ...
  for (int i = 0; i < 512; ++i) {
    ...
    if (pte & PTE_V) {
      ...
      if ((pte & (PTE_R|PTE_W|PTE_X)) == 0)
        _vmprint((pagetable_t)child, k + 1);
    }
  }
}
...
// map the user's kernel page table
void
...
// init the user's kernel page table
void
...
// free the page but the physical memory
void
ukvfree(pagetable_t pagetable)
{
  for(int i = 0; i < 512; i++){
    pte_t pte = pagetable[i];
    if(pte & PTE_V){
...
int
kuvmcopy(pagetable_t old, pagetable_t new, uint64 oldsz, uint64 newsz)
{
  pte_t *pte;
  uint64 pa, i;
  uint flags;
  ...
  return 0;
 err:
  uvmunmap(new, oldsz, (i - oldsz) / PGSIZE, 1);
  return -1;
}

uint64
kvmdealloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz, int opt)
{
  if(newsz >= oldsz)
    return oldsz;
  ...
  if(PGROUNDUP(newsz) < PGROUNDUP(oldsz)){
    int npages = (PGROUNDUP(oldsz) - PGROUNDUP(newsz)) / PGSIZE;
    uvmunmap(pagetable, PGROUNDUP(newsz), npages, opt);
  }
